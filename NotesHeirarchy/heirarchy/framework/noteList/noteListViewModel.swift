...
class NoteListViewModel : ObservableObject{
    let _getNoteListUseCase : GetNotesUseCase
    ...
    init(_getNoteListUseCase: GetNotesUseCase, _deleteSpecificUser: RemoveUserNoteUseCase, _appEnvironment : AppEnvironment) {
        self._getNoteListUseCase = _getNoteListUseCase
        self._deleteSpecificUser = _deleteSpecificUser
        self._appEnvironment = _appEnvironment
    }
    
    func onRemoveNote(note : Note) {
        Task {
            await MainActor.run{
    ...
    func getLastestNoteList() {
        ...
        Task{
            await MainActor.run{
                self.chatListState = .uiLoading(data: (self.chatListState.data ?? []) + [progressNote])
            }
            try await Task.sleep(for: .seconds(1))
            let response = await _getNoteListUseCase.invoke(pageNo: currentPageNo, pageSize: pageSize)
            switch response{
